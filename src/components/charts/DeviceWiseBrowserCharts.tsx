...
export const DeviceWiseBrowserCharts: React.FC<DeviceWiseBrowserChartsProps> = ({
  ...
  // Generate weekly pattern data from device stats
  const weeklyPatternData = useMemo(() => {
    const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']
    
    // Simulate weekly pattern based on device usage
    const weeklyData = dayNames.map((day, index) => {
      // Simulate different patterns for different days
      let visits = 0
      
      if (index === 0 || index === 6) { // Weekend
        visits = Math.floor(totalVisits * 0.12) // Lower weekend activity
      } else { // Weekday
        visits = Math.floor(totalVisits * 0.16) // Higher weekday activity
      }
      ...
      return {
        day,
        visits,
        avgDuration: 45 + Math.floor(Math.random() * 30) // 45-75 minutes average
      }
    })
